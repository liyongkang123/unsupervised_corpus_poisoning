...
import wandb
import time
def embedding_score_two(model, text_1_inputs,text_2_inputs, score_function='cos_sim'):

    # 1. Tokenize the text
    # 2. Get the embeddings
    with torch.no_grad():
        embedding_1 = model.encode_passage(move_to_cuda(text_1_inputs))[:, 0]
        embedding_2 = model.encode_passage(move_to_cuda(text_2_inputs))[:, 0]
    # 3. Calculate the similarity
    if score_function == 'cos_sim':
        score = F.cosine_similarity(embedding_1, embedding_2, dim=-1)
    elif score_function == 'dot':
        score = torch.sum(embedding_1 * embedding_2, dim=-1)
    else:
...
def main():
    ...
    wandb.init(  project="UCP_attack_generate_unsupervised", )
    ...
    assert attack_args.method == "unsupervised"
    ...
    # Sample 100 queries from query
    # query is too large to attack completely, need to sample 100 queries from it
    ...
    recon_model = BaseRecontructionModel( context_encoder=ctx_encoder, decoder=decoder)
    recon_model.transfer_linear.load_state_dict(torch.load(f"{load_model_path}/transfer_linear.pt"))

    attack_model = UnsupervisedAttack(model= recon_model, tokenizer= tokenizer , seed= training_args.seed)
    ...
    for q in query_sample:
        ...
        top_1_text = corpus[rrqt_q_top1[0]] # # There are title and text in it, you need to combine the input model
        ...
        optimizer = optim.AdamW(attack_model.noise_model.parameters(), lr=training_args.learning_rate, weight_decay=training_args.weight_decay)  # Adjust lr as needed

        adv_list = []
        target_d_inputs = tokenizer(top_1_text, return_tensors="pt", padding=True, truncation=True,encode_passage=True,
                          max_length=data_args.passage_max_len)
        target_d_inputs = move_to_cuda(target_d_inputs)
        ...
        for epoch in range(attack_args.num_attack_epochs):
            ...
            loss.backward()
            optimizer.step()
            # print('loss:',loss.item(), "mse_loss:", mse_loss.item(), "cross_entropy_loss:", cross_entropy_loss.item(), 'adv_text:', outputs['adv_text'])

            adv_text_inputs = tokenizer(outputs['adv_text'], return_tensors='pt', padding=True, truncation=True,max_length=data_args.passage_max_len,encode_passage=True)
            q_adv_score = embedding_score_two(recon_model, query_input, adv_text_inputs, score_function=score_function)
            d_adv_score = embedding_score_two(recon_model, target_d_inputs, adv_text_inputs, score_function=score_function)
            # print(f"q_adv_score: {q_adv_score}, d_adv_score: {d_adv_score},max_q_d_score: {rrqt_q_top1}")
